...
RunService.RenderStepped:Connect(function()
    for player, esp in pairs(espObjects) do
        if not player:IsDescendantOf(Players) or not player.Character or not player.Character:FindFirstChildOfClass("Humanoid") or player.Character:FindFirstChildOfClass("Humanoid").Health <= 0 then
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if not (hrp and head and humanoid) or humanoid.Health <= 0 then continue end
